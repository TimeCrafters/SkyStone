...
public class DirectionDriveAbsoute extends Drive {
    ...
    public DirectionDriveAbsoute(Engine engine, StateConfiguration stateConfig, String stateConfigID) {
        this.engine = engine;
    ...
    public void exec() throws InterruptedException {
        if (StateConfig.allow(StateConfigID)) {
            ...
            if (FirstRun) {
                ...
                Log.i("DirectionDriveAbs", "Adjusted Degrees :"+(-DirectionDegrees - robotRotation));
            ...
            if (CurrentTick >= tickDistance || currentTime - StartTime >= InterruptTime) {
                ...
                DriveForwardLeft.setPower(Power * getForwardLeftPower(-DirectionDegrees -robotRotation, 0.01));
                DriveBackRight.setPower(Power * getForwardLeftPower(-DirectionDegrees -robotRotation, 0.01));

                DriveForwardRight.setPower(Power * getForwardRightPower(-DirectionDegrees -robotRotation, 0.01));
                DriveBackLeft.setPower(Power * getForwardRightPower(-DirectionDegrees -robotRotation, 0.01));
            }
